...
class AgentRole(str, Enum):
    ...
    PROPOSER = "proposer"  # Orchestrator brain: Model A proposes decisions
    CRITIC = "critic"  # Orchestrator brain: Model B critiques proposals
...
class MessageType(str, Enum):
    ...
    # Orchestrator brain flow: propose → critique → synthesize
    PROPOSAL = "proposal"  # Proposer → Critic: initial decision proposal
    CRITIQUE = "critique"  # Critic → Proposer: critique of proposal
    SYNTHESIS = "synthesis"  # Proposer → (final): synthesized decision
...
def create_proposal(
    ...
    """Create a proposal message from Proposer to Critic (brain Phase 1)."""
    return A2AMessage(
        message_type=MessageType.PROPOSAL,
        from_role=AgentRole.PROPOSER,
        to_role=AgentRole.CRITIC,
        task_id=task_id,
        payload={
...
def create_critique(
    ...
    """Create a critique message from Critic to Proposer (brain Phase 2)."""
    return A2AMessage(
        message_type=MessageType.CRITIQUE,
        from_role=AgentRole.CRITIC,
        to_role=AgentRole.PROPOSER,
        task_id=task_id,
...
def create_synthesis(
    ...
    """Create a synthesis message (final decision from Proposer after critique)."""
    return A2AMessage(
