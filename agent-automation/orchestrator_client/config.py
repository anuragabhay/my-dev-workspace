...
def get_llm_model() -> str:
    """LLM model name (fallback when proposer/critic not set)."""
...
def get_proposer_model() -> str:
    """Model for orchestrator brain Proposer (Phase 1 and 3)."""
    return os.environ.get(
...
def get_critic_model() -> str:
    """Model for orchestrator brain Critic (Phase 2)."""
    return os.environ.get(
        "ORCHESTRATOR_CRITIC_MODEL",
        get_llm_model(),
    )
